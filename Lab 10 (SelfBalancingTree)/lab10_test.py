"""Test File for Lab 10 and bst_rb
   Parth Ray
"""

import unittest
from bstree_rb import *
from lab10 import *
...
class TestCase(unittest.TestCase):
    ...
    def test_preorder_list(self):
        tree = insert(None, 5, "P")
        tree = insert(tree, 4, "A")
        tree = insert(tree, 3, "R")
        self.assertEqual(preorder_list(tree), [4, 3, 5])

    def test_tree_height(self):
        tree = insert(None, 0, "P")
        self.assertEqual(tree_height(tree), 0)
        tree = insert(tree, 1, "A")
        tree = insert(tree, 2, "R")
        self.assertEqual(tree_height(tree), 1)
        tree = insert(tree, 3, "T")
        tree = insert(tree, 4, "H")
        tree = insert(tree, 5, "R")
